...
void sapflow_loop () {
  ...
  if (currentMillis - startMillis >= senddelay) {
     ...
     startMillis = currentMillis; // I MISSED THIS LINE
  }
  heat();
}
...
void package_sapflow(OSCBundle *bndl, char packet_header_string[]) 
{
  ...
  sprintf(address_string, "%s%s", packet_header_string, "/temp0");
  bndl->add(address_string).add(state_sapflow.temp0);
  sprintf(address_string, "%s%s", packet_header_string, "/temp1");
  bndl->add(address_string ).add(state_sapflow.temp1);
  sprintf(address_string, "%s%s", packet_header_string, "/temp_diff");
  bndl->add(address_string).add(state_sapflow.temp_diff);
}
